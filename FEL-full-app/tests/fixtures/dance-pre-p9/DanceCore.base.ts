...
// app code and never edit it. Everything below this header is byte-for-byte
// `git show 7ee51e4e:FEL-full-app/lib/babylon/core/DanceCore.ts`: the dance judge as it stood before movement play
// phase 9 added body steps. The equivalence test drives it and the live DanceCore.ts with the same button input and
// requires the same judgements, score, combo, accuracy and result, which is how "button play is unchanged" is proved.
// It lives under tests/, not lib/: lib/nav/modules.test.ts rightly counts a lib module that only a test imports as dead.
// ─────────────────────────────────────────────────────────────────────────────────────────────────────────────────────
