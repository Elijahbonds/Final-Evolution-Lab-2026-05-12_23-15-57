// FROZEN PRE-P7 COPY: a test fixture, for lib/babylon/combat/padEquivalence.test.ts ONLY. Never import it from app
// code and never edit it. Everything below this header is `git show b440b737:FEL-full-app/lib/babylon/core/StrikeSystem.ts`: the
// combat core as it stood before movement play phase 7 added the body's timing seam (a strike's elapsed start, the
// string's pressMove, the widened body windows, the slip). The equivalence test drives it and the live file with the
// same PAD input and requires the same answers, which is how "pad play is unchanged" is proved (PLAN-P7 G5).
// (its three relative imports point at the frozen FightCore / HordeDynamics copies and at the live, unchanged boneLookup)
...
import type { AttackDef } from './FightCore.base';
import { boneNode, findBone } from '@/lib/babylon/anim/boneLookup';
import { MOVES, STRIKE_TIMING, attackFromMove, type HordeMove } from './HordeDynamics.base';   // phase 4: the book
