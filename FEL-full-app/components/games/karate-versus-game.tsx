...
export default function KarateVersusGame({ grade, prq, onEnd, gamepad }: GameProps) {
  ...
  useEffect(() => {
    ...
    const loop = (now: number) => {
      ...
      // msgTimer/roundMsgTimer/aiTimer gate the round transition and the AI's
      // entire decision loop below — they should still respect the dragon/
      // hit-stop slow-mo (that's the intended drama), but must NOT ride the
      // 0.05-capped rawDt on top of it, or rAF throttling (backgrounded tab,
      // low-power mode) can leave the match frozen after every round banner.
      const unstickDt = (dragon.active ? wallDt * dragon.timeScale : wallDt) * hitScale;
      ...
      if (msgTimer > 0) msgTimer -= unstickDt;
      if (roundMsgTimer > 0) roundMsgTimer -= unstickDt;
