...
export default function TrainingGame({ grade, prq, onEnd, gamepad }: GameProps) {
  ...
  useEffect(() => {
    ...
    const loop = (now: number) => {
      ...
      last = now;
      // Round clock and message timer are pure countdowns — track real
      // elapsed time so rAF throttling (backgrounded tab, low-power mode)
      // can't leave the round looking frozen at 60s forever.
      if (!endedRef.current) {
        timeLeft -= rawDt;
