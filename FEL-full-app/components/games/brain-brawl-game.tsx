...
export default function BrainBrawlGame({ grade, prq, onEnd, gamepad }: GameProps) {
  ...
  const spinRef = useRef<any>({ angle: 0, vel: 0, spinning: false, target: -1, startedAt: 0 });
  ...
  const startSpin = useCallback(() => {
    ...
    spinRef.current.startedAt = performance.now();
  }, []);
  ...
  useEffect(() => {
    ...
    const loop = (now: number) => {
      ...
      if (sp.spinning) {
        ...
        // Deceleration is paced per rAF frame, not wall-clock time — if frames
        // arrive in a throttled burst (backgrounded tab, low-power mode) the
        // velocity barely decays no matter how much real time has passed,
        // leaving the wheel spinning indefinitely. Force a stop once the spin
        // has run long enough regardless of how many frames were delivered.
        if (sp.vel < 0.4 || now - sp.startedAt > 4000) {
