...
export default function ThreePointGame({ grade, prq, onEnd, gamepad }: GameProps) {
  ...
  useEffect(() => {
    ...
    const loop = (now: number) => {
      ...
      last = now;
      // The 60s round clock and message timer are pure countdowns with no
      // physics to destabilize, so they track real elapsed time — under rAF
      // throttling (backgrounded tab, low-power mode) the 0.05s-clamped dt
      // could take dozens of frames to burn even one real second, making the
      // round look frozen instead of just ending on time.
      if (!endedRef.current) {
        timeLeft -= rawDt;
