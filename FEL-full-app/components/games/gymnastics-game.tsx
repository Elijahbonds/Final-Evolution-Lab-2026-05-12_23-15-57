...
export default function GymnasticsGame({ grade, prq, onEnd, gamepad }: GameProps) {
  ...
  useEffect(() => {
    ...
    const loop = (now: number) => {
      ...
      last = now;
      // cadenceTimer/landMsgTimer are pure display timers; core.step's
      // second argument drives its internal 'Land' phase wait (see
      // air-session-core.ts) — all three need real elapsed time so rAF
      // throttling can't leave a vault attempt stuck after landing.
      if (cadenceTimer > 0) cadenceTimer -= rawDt;
      if (landMsgTimer > 0) landMsgTimer -= rawDt;
