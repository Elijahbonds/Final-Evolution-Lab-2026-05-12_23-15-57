...
export default function ThreeVThreeGame({ grade, prq, onEnd, gamepad }: GameProps) {
  ...
  useEffect(() => {
    ...
    const loop = (now: number) => {
      ...
      last = now;
      // Round clock, AI possession tick, shot-clock timer, and message timer
      // are pure countdowns — track real elapsed time so rAF throttling
      // (backgrounded tab, low-power mode) can't leave the round frozen.
      if (!endedRef.current) {
        timeLeft -= rawDt;
