...
export default function SprintGame({ grade, prq, onEnd, gamepad }: GameProps) {
  ...
  useEffect(() => {
    ...
    const loop = (now: number) => {
      ...
      last = now;
      // Clamp physics dt to guard against a single huge frame gap destabilizing
      // the sim (teleporting the runner) — but that risk only exists once the
      // race is actually moving (Go/Run). The pre-race Ready/Set clock and the
      // false-start message timer are pure time-based state machines with no
      // physics to blow up, so under rAF throttling (backgrounded tab, low-power
      // mode) they should still track real elapsed time — otherwise a false
      // start can look permanently stuck for as long as frames stay throttled.
      const racingPhase = core.state.phase === 'Go' || core.state.phase === 'Run';
      const dtMs = racingPhase ? Math.min(rawDtMs, 250) : rawDtMs;
