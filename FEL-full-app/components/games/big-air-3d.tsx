...
function BigAirScene({
  ...
  useFrame(({ camera }, dtRaw) => {
    ...
    // dtRaw (uncapped) drives the core's FSM timing and the message timer —
    // both are pure countdowns with no physics to destabilize, so under rAF
    // throttling (backgrounded tab, low-power mode) they should still track
    // real elapsed time rather than inherit the 0.05s physics clamp.
    const s = c.step(dt, dtRaw);

    if (v.msgT > 0) v.msgT -= dtRaw;
