...
export default function GolfGame({ grade, prq, onEnd, gamepad }: GameProps) {
  ...
  useEffect(() => {
    ...
    const loop = (now: number) => {
      ...
      last = now;
      // msgTimer/resultTimer gate advancing to the next shot (or finishing
      // the round) below — track real elapsed time so rAF throttling
      // (backgrounded tab, low-power mode) can't leave the round stuck on
      // the result screen forever.
