...
| **Post-processing — SSAO, bloom, tone mapping** | `lib/babylon/visual/RenderPipeline.ts` is a **complete `DefaultRenderingPipeline` configuration** — FXAA, bloom (per-mood threshold/weight/scale), exposure, contrast, saturation curves — **and nothing mounts it.** A whole-tree grep finds no consumer. `NexusWebScene` sets `toneMappingEnabled` only. SSAO: none anywhere (`RigValidationScene` explicitly declines it as too expensive). | **The directive's "highest impact for lowest risk" item is already written and switched off.** Mounting it in `ModeHarness` is a one-line change plus a frame-budget check per mode. |
| **Dynamic shadows — cascaded** | `ShadowGenerator(1024)` in `NexusWebScene` venues; `IblShadowsRenderPipeline` (Babylon 9) behind `NEXT_PUBLIC_IBL_SHADOWS`. No `CascadedShadowGenerator`. | **Partial.** Venue-spec modes have shadows; ride worlds and the dojo builders do not. |
...
| 1 | **Mount `RenderPipeline`** in `ModeHarness` (bloom + tone mapping + FXAA already configured per mood) | High | Trivial + a per-mode fps check | Already written |
