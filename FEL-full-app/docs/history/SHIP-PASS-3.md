...
## Findings log
- **4 Sep, the swap.** The owner ran the swap commands: `public/models/fel-hero.glb` is the male kit body (identical bytes to `candidates/fel-kit-male.glb`), the eight rivals under `public/models/athletes/` are the kit-derived roster (0.8–1.2 MB each), and the forge hero and roster live on under `public/models/_forge/`. The definitive gate, the full gauntlet on the shipping path, runs next; its row count goes in the commit.
