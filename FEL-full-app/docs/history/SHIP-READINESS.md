...
|---|---|---|
| Every Babylon mode plays on its shipping route, logged in | `scripts/gauntlet-play.sh`: 20/20 0/0/0 (dev server); 20/20 0/0/0 twice on the production bundle (:3004), the second on the release build | ✅ |
...
| Mobile tier and phone controls | 7 modes at 59–61 fps under 4× CPU throttle; phone captures 7 modes, 0 errors, on the release build | ✅ |
...
Tag: `v0.9.0-rc.1` (local; not pushed). Signed: Claude (build), pending the owner's two items above. Not "shipped": the per-mode benchmark comparisons live in `docs/concept-lock/*-signoff.md`.
