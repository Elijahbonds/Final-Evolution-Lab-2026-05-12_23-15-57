...
- **`/play/dunkduel` is PROVE IT**, the IRL camera dunk contest (real footage, on-device pose tracking), not a Babylon mode. Controls measures pad latency, Logic measures a masher against intent and Body measures a rig's clips — it has none of the three, and N/A on all six is neither a pass nor a judgement. Since 2026-09-15 it is scored on its **flow** instead (`scripts/probes/_proveit-flow.mts`: the page comes up and publishes `#fel-ready`, consent reaches the camera, the track goes live, the console stays clean), which stands in for Controls and Logic; Visuals, Feel and Performance come from the same frames and fps as every other game, and Body reports N/A with its reason. Chromium's fake capture device plays a synthetic pattern, so no pose is ever found and no dunk is judged: the flow is measured as far as "armed and tracking", and a human still has to stand in front of a real camera to review the judging itself. The Babylon duel it replaced still runs at `/dev/mode/dunkduel`.
