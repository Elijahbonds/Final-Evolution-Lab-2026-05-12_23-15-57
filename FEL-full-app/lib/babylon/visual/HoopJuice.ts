...
//   #1 rim spring  — a juice-only torus (`juice_rim`) at the play rim, shown for the beat: XZ 1.0 → 1.06 → 1.0, damped, ~140 ms
//   #2 net squash  — a juice-only wireframe cylinder (`juice_net`) hung from the rim: y 1 → ~0.7 → 1 with a light sway, ~300 ms
...
export class HoopJuice {
  ...
  private tick(dt: number): void {
    ...
    this.t += dt;
    const t = this.t;
    // #1 damped spring on the ring's XZ
    if (t < SPRING_S) { const k = t / SPRING_S; const s = 1 + 0.06 * Math.sin(k * Math.PI) * (1 - 0.4 * k); this.rimRing.scaling.set(s, 1, s); }
    else if (this.rimRing.isVisible) { this.rimRing.scaling.set(1, 1, 1); this.rimRing.isVisible = false; }
    // #2 net squash + sway
    if (t < NET_S) { const k = t / NET_S; this.netPivot.scaling.y = 1 - 0.3 * Math.sin(k * Math.PI) * (1 - 0.3 * k); this.netPivot.rotation.z = 0.08 * Math.sin(k * Math.PI * 2) * (1 - k); }
