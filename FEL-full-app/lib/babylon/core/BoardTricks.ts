...
/**
 * AIR TIMES RESCALED (BOARD-10PHASE P5). They used to run 0.25-0.72 s against a pop giving 0.93-1.31 s of hang,
 * so every trick in the table fitted off flat ground and `fitsAir` gated nothing: the list was 15 long and
 * choosing from it was not a decision. The THPS benchmark asks that the longest trick need ~80% of maximum hang.
 *
 * Now, against skate's 0.93 s (no charge) / 1.12 s (half) / 1.31 s (full):
 *
 *   no charge   ollie, shuv-it, indy, melon, kickflip, heelflip, backside 180
 *   half        + japan air, frontside 360, 360 flip
 *   full        + the 540
 *
 * The 540 at 1.24 s is 95% of maximum hang, so it exists only off a fully charged pop off something. Nothing
 * here changes what a trick IS — only how much air it needs to finish, which is the number the landing already
 * grades against.
 */
export const SKATE_TRICKS: readonly BoardTrick[] = [
  T({ id: 'ollie', label: 'OLLIE', discipline: 'skate', kind: 'air', dir: null, btn: 'A', spinDeg: 0, flipDeg: 0, grab: 'none', difficulty: 1.0, airSec: 0.34, clip: 'board_air' }),
  T({ id: 'kickflip', label: 'KICKFLIP', discipline: 'skate', kind: 'air', dir: 'left', btn: 'A', spinDeg: 0, flipDeg: 360, grab: 'none', difficulty: 1.6, airSec: 0.58, clip: 'skate_kickflip' }),
  T({ id: 'heelflip', label: 'HEELFLIP', discipline: 'skate', kind: 'air', dir: 'right', btn: 'A', spinDeg: 0, flipDeg: -360, grab: 'none', difficulty: 1.7, airSec: 0.58, clip: 'skate_kickflip' }),
  T({ id: 'shuvit', label: 'SHUV-IT', discipline: 'skate', kind: 'air', dir: 'down', btn: 'A', spinDeg: 180, flipDeg: 0, grab: 'none', difficulty: 1.4, airSec: 0.46, clip: 'board_air' }),
  T({ id: 'tre', label: '360 FLIP', discipline: 'skate', kind: 'air', dir: 'down', btn: 'B', spinDeg: 360, flipDeg: 360, grab: 'none', difficulty: 3.2, airSec: 1.1, clip: 'skate_kickflip' }),
  T({ id: 'indy', label: 'INDY', discipline: 'skate', kind: 'air', dir: 'up', btn: 'B', spinDeg: 0, flipDeg: 0, grab: 'indy', difficulty: 1.5, airSec: 0.52, clip: 'board_grab' }),
  T({ id: 'melon', label: 'MELON', discipline: 'skate', kind: 'air', dir: 'left', btn: 'B', spinDeg: 0, flipDeg: 0, grab: 'melon', difficulty: 1.6, airSec: 0.52, clip: 'board_grab' }),
  T({ id: 'japan', label: 'JAPAN AIR', discipline: 'skate', kind: 'air', dir: 'up', btn: 'Y', spinDeg: 0, flipDeg: 0, grab: 'japan', difficulty: 2.6, airSec: 0.96, clip: 'board_grab' }),
  T({ id: 'bs180', label: 'BACKSIDE 180', discipline: 'skate', kind: 'air', dir: 'right', btn: 'B', spinDeg: 180, flipDeg: 0, grab: 'none', difficulty: 1.8, airSec: 0.66, clip: 'board_air' }),
  T({ id: 'fs360', label: 'FRONTSIDE 360', discipline: 'skate', kind: 'air', dir: 'right', btn: 'Y', spinDeg: 360, flipDeg: 0, grab: 'none', difficulty: 2.6, airSec: 1.04, clip: 'board_air' }),
  T({ id: 'spin540', label: '540', discipline: 'skate', kind: 'air', dir: 'left', btn: 'Y', spinDeg: 540, flipDeg: 0, grab: 'indy', difficulty: 3.6, airSec: 1.24, clip: 'board_grab' }),
  // ground links
...
/**
 * P5: snow's hierarchy already worked — its pop gives 1.11-1.50 s and its longest trick wanted 1.15 s, which is
 * 77% of maximum hang against the SSX benchmark's 80%. The top two are nudged to 1.22 and 1.14 so the biggest
 * trick needs 81% of a full tuck and the floor no longer throws nine of ten. Deliberately a nudge and not a
 * rescale: snow was the discipline the other two were measured against, and it earned that by being right.
 */
export const SNOW_TRICKS: readonly BoardTrick[] = [
  ...
  T({ id: 'snow720', label: '720', discipline: 'snow', kind: 'air', dir: 'right', btn: 'Y', spinDeg: 720, flipDeg: 0, grab: 'indy', difficulty: 4.4, airSec: 1.14, clip: 'board_grab' }),
  T({ id: 'cork720', label: 'CORK 720', discipline: 'snow', kind: 'air', dir: 'left', btn: 'Y', spinDeg: 720, flipDeg: 180, grab: 'melon', difficulty: 5.2, airSec: 1.22, clip: 'board_grab' }),
...
export const SURF_TRICKS: readonly BoardTrick[] = [
  ...
  // P5: surf had three airs. ONE more, not two — and the reason is a test already in this suite: "a surf list is
  // mostly ON the wave, not in the air — a wave is not a ramp". My Phase 2 criterion asked for five airs, which
  // would have made surf 5 air against 5 carve and broken that claim. The codebase's existing design intent is
  // more specific than my number was, so the criterion moved instead of the design. Four airs, spread 0.35-0.78.
  T({ id: 'rodeo', label: 'RODEO FLIP', discipline: 'surf', kind: 'air', dir: 'down', btn: 'Y', spinDeg: 180, flipDeg: 360, grab: 'none', difficulty: 4.6, airSec: 0.78, clip: 'board_air' }),
];
