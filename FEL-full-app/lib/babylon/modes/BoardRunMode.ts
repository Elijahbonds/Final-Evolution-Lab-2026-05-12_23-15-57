...
export function makeBoardMode(cfg: BoardConfig): ModeDefinition {
  ...
  // 'descent' and 'surf' are purpose-tuned presets (wider/higher for the big
  // slope airs; low and close for wave-following) that sat unused — every
  // board mode was hardcoded to the generic 'board' preset, which is sized
  // for skateboard's tighter park and was letting the camera lose the rider
  // on snowboard's much larger, sloped course.
  const camPreset = cfg.modeId.includes('snow') ? 'descent' : cfg.modeId === 'surf' ? 'surf' : 'board';

  return {
    modeId: cfg.modeId, mood: cfg.mood, camPreset,
