...
import { attachBallToHand, releaseBall, runHandOffPath, handOffK, clankOffRim, palmOffsetOf, HAND_OFF_BLEND, EASTBAY_PASSES, handOffSpecAt, type HandOffSpec } from '../anim/ballRig';
...
import { DunkFlight, DunkSpin, SPIN_720, SPIN_720_UPGRADE_BY, type SpinRecord, runwayTrickFor, cueOf, cueVerdict, cueFireAt, cueLastAt, CUE_BEAT_LABEL, SPIN_RESOLVE_T, runwayTeachLine, CATCH_DIFFICULTY, DUNK_TRICK_ID_BY_CLIP, type RunwayTrick, type DunkTrick } from '../core/DunkSystem';
...
import { OBSTACLE_SPECS, OBSTACLE_KINDS, PROP_CAM, clipsObstacle, heightAt, nextObstacle, propCamSpot, propCutDue, isDubble, DUBBLE_HEAD_TOP, DUBBLE_APEX_M, dubbleArc, dubbleArcPower, type ObstacleKind } from '../core/DunkObstacles';
...
/**
 * THE CURVED APPROACH (DUNK MOTION phase 7; owner, 2026-09-23: "have the model curve their approach. have the body able to open
 * up to the side, look at takeoff posture and in flight body mechanics").
 *
 * The hold-run drove STRAIGHT down the rim's line, and it pulled any offset back onto it at up to 2 m/s, so every dunker arrived
 * square to the iron, bolt upright, and left the floor already facing it. A real dunker runs a J (a "banana"): out on the
 * ball-hand side, then curving in over the last strides so the plant comes in ACROSS the rim at an angle, the body LEANING into
 * the curve the way any runner banks a bend, and the hips and chest OPEN to the rim through the rise instead of starting there.
 * The curve turns run speed into rotation you can see, and it is how the take-off reads as an athlete's rather than a sled's.
 *
 * The hold-run's lateral target is the J (curveOffset); the stick still steers on top of it. The body banks with the real
 * lateral acceleration of the path (atan(a/g), capped), the bank carries off the floor and unwinds through the rise, the flight
 * is a STRAIGHT line from the plant to the rim (a jump cannot curve), and the root opens onto the rim over the rise rather than
 * snapping to it. The judges score the angle the PLAYER chose: the curve's own offset is taken out of the approach read.
 */
const CURVE_WIDE_M = 1.6, CURVE_LINE_M = 0.8, CURVE_FROM_M = 5.5;
/** The most the whole body leans into the curve, and how fast the lean follows the path. */
const RUN_BANK_MAX = 14 * Math.PI / 180, RUN_BANK_TAU = 0.12;
/** The flight opens onto the rim at this yaw rate through the rise (the old FACE_RIM_RATE 6 squared it up in ~0.25 s). */
const FACE_RIM_RATE_RISE = 2.6;
...
/** DUNK MOTION phase 10b: the kick-up's ball sits this far above the instep on the kicking foot (a ball's radius less the shoe). */
const BALL_R_KICK = 0.1;
...
const PROPS = ['none', 'alleyoop', 'oopglass', 'oopbounce', 'oopcorner', 'oopalien', 'selflob', 'offglass', 'bounce', 'car', 'barrier', 'crate', 'tetris', 'ladder', 'bike', 'bikeroll', 'skate', 'skateroll', 'row3', 'row5', 'wall', 'kangaroo', 'dubble1', 'dubble2', 'dubble3', 'dubble4', 'dubble5', 'dubble6', 'dubble7', 'dubble8', 'dubble9', 'dubble10'] as const;   // DUNK MOTION phase 10: the DUBBLE UP over 1–10
...
/** Dev: `?prop=<id>` starts the night on that prop (the motion probe's Dubble Up runs). */
const DEV_PROP = process.env.NODE_ENV === 'development' && typeof location !== 'undefined' ? (location.search.match(/[?&]prop=([a-z0-9]+)/)?.[1] ?? null) : null;
/** DUNK MOTION phase 10 — THE DUBBLE UP: the ball hand starts down for the ball this far before the helper; it takes it within this gap
 *  (or as the body passes him); the legs open this far around his head. */
const DUBBLE_REACH_FROM_M = 1.1, DUBBLE_REACH_LEAD_SEC = 0.55, DUBBLE_GRAB_M = 0.26, STRADDLE_DEG = 34;
...
export const DunkMode: ModeDefinition = (() => {
  ...
  /** DUNK MOTION phase 10b: the eastbay's two passes (ballRig.EASTBAY_PASSES) — into the off hand up the front, back under the thigh
   *  to the dunking hand — on the right-handed body. */
  const EASTBAY_HANDOFFS: HandOffSpec[] = EASTBAY_PASSES.map((sp) => ({ ...sp, from: hand(sp.from), to: hand(sp.to) }));
  ...
  // DUNK MOTION phase 10b: to the DUNKING hand — the off hand takes it up the front (FRONT_SWAP_SEC), the transfer hands it back
  const FRONT_SWAP: HandOffSpec = { at: FRONT_SWAP_AT, from: hand('RightHand'), to: hand('LeftHand'), blend: FRONT_SWAP_BLEND };
  const BETWEEN_LEGS_SPECS: HandOffSpec[] = [FRONT_SWAP, { at: BETWEEN_LEGS_HANDOFF, from: hand('LeftHand'), to: hand('RightHand') }];
  /**
  ...
  const TRICK_HANDOFFS: Record<string, HandOffSpec[]> = {
    lostfound: [LOST_FOUND_SPEC], betweenlegs: BETWEEN_LEGS_SPECS,
    eastbay: EASTBAY_HANDOFFS,
    behindback: [FRONT_SWAP, { at: BEHIND_BACK_SWAP, from: hand('LeftHand'), to: hand('RightHand') }],
  ...
  // DUNK MOTION phase 10b: the scorpion ends LOADED (the ball behind the ducked head, the heels up) and waits in that shape for the
  // SLAM — flowed into the carry-up, the carry stood the body up and the jam was a plain one-hander (measured, p10b-base)
  const WAITING_CLIPS = new Set<string>([CARRY.Right, CARRY.Left, CARRY.Both, SPORT_CLIP.dunkScoreHang, 'dunk_scorpion']);
  /** Called tricks that jam in their own shape (phase 10b): the scorpion goes over the back of the ducked head. */
  const TRICK_FLUSH: Record<string, string> = { scorpion: 'dunk_flush_scorpion' };
  ...
  // phase 10b: the scorpion is READY at its loaded key (0.5) — the rest is the hold it waits in; paced on its whole 0.8 it ran 1.6×
  // and the swing from the hip up behind the head happened inside the crossfade
  const TRICK_READY_AT: Record<string, number> = { eastbay: EB.extend, scorpion: 0.5 };
  ...
  const launchMult = () => launchProfile(isDubble(prop) ? 'two' : takeoffFor(runUpPeak, gatherHeld), vectorLive(), vectorWallRun).apexMult;
   ...
   *  every later A in that flight is ignored. (HOTFIX 2026-09-24: the rule lives in core/slamPress's SlamLatch now, unchanged,
   *  so it can be driven without Babylon; it also holds the press waiting for the window.) */
  const slamLatch = new SlamLatch();
  /** HOTFIX (2026-09-24): the A that took off is the take-off's, never the slam. The keyboard's take-off is the Space release,
   *  which InputBus sends as R 0 and then an A (tagged `src: 'space'`), and a pad's A on the run is the take-off itself: either A
   *  reached the flight as its FIRST press, was held, and was refused TOO EARLY at the window, and the real slam was then
   *  ignored. The same for the "tap JUMP" that lands just after the line has launched the run on its own. */
  ...
  let liveTricks: { clip: string; t0: number; speed: number }[] = [], liveSpin: SpinRecord = { turns: 0, from: 0, until: 0 };   // this attempt's air tricks, for the replay
  ...
  const def: ModeDefinition = {
    ...
    stakes = freshStakes();
      ...
      setPhase('approach');
      startWalkOut();
      if (prop !== 'none') void setupProp(ctx);   // (dev ?prop=: the first attempt of the night stands on it too)
      ctx.setHud({
    ...
    onInput(ctx: ModeContext, e: FelInput) {
      ...
      // Y = SELF-LOB (also standing, in the approach), B = KICK-UP, X = BACK HANDSPRING (it tosses the lob itself), X + down = CARTWHEEL, A = DOUBLE-UP
      ...
      if (e.t === 'button' && e.pressed && e.btn === 'Y' && phase === 'charge' && !yHeldDir) commitCurve(ctx);
        ...
        // Y + up/down picks the lob variant (off the glass, off the bounce); B + up is the BACKFLIP (owner, 2026-09-16); X + down is Jus
        // Fly's CARTWHEEL (phase 10b). A direction with no variant on that button falls back to the plain trick (runwayTrickFor), so a
        // held stick-hand never turns a kick-up into something else.
        const variant = e.btn === 'Y' ? (held ?? (prop === 'offglass' ? 'up' : prop === 'bounce' ? 'down' : null)) : held;
        ...
        // DUNK MOTION phase 10 (owner decision): A on the run is the TAKE-OFF — over the Dubble Up's helper, or from wherever you are. The
        // runway's two-foot hop was a misreading of "double-up" and is gone.
        if (e.btn === 'A' && phase === 'charge') {
          if (!runwayBeat) launchDunk(ctx, 'press');
      ...
      // HOTFIX (2026-09-24, BASELINE.md:245): the take-off's own A is not the slam. launchDunk moves the phase to 'cinematic' on the
      // spot, so the A that jumped (a pad's A on the run: this same event; the keyboard's Space release: the A InputBus sends right
      // after its R 0) came on down to airButton and was buffered as the SLAM at flight clock 0 — refused TOO EARLY at the window,
      // and, the first press deciding, every real slam after it ignored. Read AFTER the launch above (core/slamPress TakeoffEcho):
      //   'takeoff' / 'late' — the A that jumped, or the "tap JUMP" landing just after the line launched the run itself. It still
      //     arms a trick under a fresh direction (the rival throws his first at clip 0.06); it is never the slam.
      //   'space' — the Space coming up (tagged by InputBus) before the SLAM read is up: the run key, not an air press at all. Once
      //     the read is up (slamCueOn: the buffer's reach through the window) letting go of Space IS a slam press, as it always was.
      const echoA = phase === 'cinematic' ? slamEcho.of(e, performance.now(), slamCueOn) : null;
      if (echoA === 'space') console.info(`[DUNK-SLAM] the Space release's own A @${clipTime.toFixed(2)} — the run key, not the slam`);
      if (phase === 'cinematic' && e.t === 'button' && e.pressed && (e.btn === 'A' || e.btn === 'B' || e.btn === 'X' || e.btn === 'Y') && !qteWindowOpen && echoA !== 'space') airButton(ctx, e, echoA !== null);   // X reads in the air
      ...
      if (e.t === 'button' && e.btn === 'A' && e.pressed && qteWindowOpen && !lob.live && !echoA) { if (phase === 'cinematic' && slamLatch.press(clipTime, true) === 'spent') console.info(`[DUNK-SLAM] a second press @${clipTime.toFixed(2)} ignored — the first press decides`); else slamNow(ctx, clipTime); }
    ...
    update(ctx: ModeContext, dt: number) {
      ...
      if (phase === 'charge' && !busRun) {
        ...
        {
          const ax = dt > 0 ? (steer - prevRunVx) / dt : 0, az = dt > 0 ? (-runNow - prevRunVz) / dt : 0; prevRunVx = steer; prevRunVz = -runNow;
          const sp = Math.hypot(steer, runNow) || 1, fx = steer / sp, fz = -runNow / sp;
          const aLat = ax * -fz + az * fx;   // across the path, + to its left-hand normal (−fz, fx)
          const yaw = player.root.rotation.y, tipX = -Math.cos(yaw), tipZ = Math.sin(yaw);
          const toward = Math.sign((-fz * tipX + fx * tipZ) * aLat) * Math.min(RUN_BANK_MAX, Math.atan2(Math.abs(aLat), 9.81));
          runBank += (toward - runBank) * lowPassK(dt, RUN_BANK_TAU);
          player.root.rotation.z = runBank;
        }
      ...
      if (dribble) {
        ...
        const ballOurs = !lob.live && !lob.thrown && !isOop(prop) && !isDubble(prop) && !runwayBeat && !launchQueued;   // (the Dubble Up: the ball is on the helper's head)
      ...
      if (phase === 'cinematic') {
        ...
        { const was = spin.yaw, turning = spin.record.turns !== 0; spin.update(clipTime);   // the momentum-led turn rides the flight's own clock (the hang slow-mo stretches both)
          // phase 10b: the turn's own telemetry — how far it went and its biggest single-frame step (the 720 is two turns in one window)
          if (turning) { spinPeak = Math.max(spinPeak, Math.abs(spin.yaw)); if (spin.yaw !== 0) spinStepMax = Math.max(spinStepMax, Math.abs(spin.yaw - was)); }
          if (turning && spin.record.turns === 0) { console.info(`[DUNK-CUE] spin landed: ${(spinPeak * 180 / Math.PI).toFixed(0)}° turned, peak step ${(spinStepMax * 180 / Math.PI).toFixed(1)}°/frame`); spinPeak = 0; spinStepMax = 0; } }
        ...
        if (!lob.live) {
          ...
          if (airTrick && specs) {
            ...
            const spec = handOffSpecAt(specs, t);
            ...
            const spec = handOffSpecAt(EASTBAY_HANDOFFS, clipTime);
            activeHandOff = { spec, t: clipTime };
            if (runHandOffPath(ball, player.skeleton, clipTime, spec, ebState)) console.info(`[HANDS] handoff ${spec.from[0]}→${spec.to[0]} eastbay @${clipTime.toFixed(2)}`);
          }
        }
        ...
        player.root.position.y = (obstacle?.dubble ? dubbleHeight(clipTime) : arcHeight(clipTime, EASTBAY_TIMING.extend)) * apexFor();   // DUNK MOTION phase 9: the top AT the rim (DunkLegs.arcHeight) · phase 10: the Dubble Up's own curve
        ...
        // DUNK MOTION phase 7: a STRAIGHT line from the plant to the rim (a jump cannot curve — the old pull toward rim.x bent the
        // flight's ground path); the stick's drift rides on top and fades by the flush, so the contact math is untouched
        player.root.position.x = launchX + (rim.x - launchX) * u + airDriftX * (1 - u);
        ...
        // the curve's bank carries off the floor and unwinds through the rise (DUNK MOTION phase 7)
        player.root.rotation.z = -airLean * AIR_LEAN_RAD + launchBank * Math.max(0, 1 - clipTime / 0.55);
        airDriftX += airLean * AIR_DRIFT * dt;
        // an angled run-up launches yawed off the iron: the body OPENS onto it through the rise (phase 7: slower there, so the
        // turn is something the hips and chest do in the air rather than a snap at the take-off), square by the hang
        faceToward(rim, dt * (clipTime < EASTBAY_TIMING.rise + 0.35 ? FACE_RIM_RATE_RISE : FACE_RIM_RATE));
        ...
        if (obstacle && !obstacleClipped) {
          ...
          if (clipsObstacle(obstacle.profile, fy, px, pz, obstacle.spec.clearance)) {
            ...
            clipBlown(ctx);
          } else if (h === 0 && obstacleOver && !obstacleCleared && pz < obstacle.farZ && (!obstacle.dubble || dubblePassed)) {
            obstacleCleared = true; onObstacleCleared(ctx);
          }
        }
        if (obstacle?.dubble && !obstacleClipped) dubbleTick(ctx, dt);   // DUNK MOTION phase 10: over the helper, the grab
        ...
        // (resolveDunk moves the phase; the resolve block below picks the jam up on this same frame. The hold is measured from the
        // window's EDGE, not the frame that crossed it: a press at the top missed by one frame's overshoot, 316 ms against 310)
        ...
        if (qteWindowOpen && !wasOpen && !lob.live) {
          const held = slamLatch.open(openAt, holdSec);
          if (held && 'slamAt' in held) slamNow(ctx, held.slamAt);
          else if (held) refuse(ctx, `TOO EARLY — ${Math.round(held.tooEarlySec * 1000)} ms BEFORE THE WINDOW`);
        }
      ...
      if (replaying && !cutting) {
        ...
        if (!replayAir && y > 0.05 && replayPrevY <= 0.05) {
          ...
          const rg = playClip(launchClip(), { speedRatio: replayRate, fadeSec: 0.25, onEnd: () => { if (replaying && replayAir && !replayAerial) { playClip(carryClip(), { speedRatio: replayRate, fadeSec: CARRY_FLOW_FADE_SEC, onEnd: () => {} }); console.info('[HANDS] replay launch → carry'); } } });
  ...
  function pickAerialFinish(hit: boolean, acc: number, leftHand = false, called: string | null = null): string {
    ...
    if (called && TRICK_FLUSH[called]) return TRICK_FLUSH[called];
    if (called || lob.caught) return FLUSH.Right;
    return launchFoot === 'one' ? FLUSH.Right : FLUSH.Both;   // the plain flight flushes the way it went up (phase 7): one hand off one foot, two off two
  }
  function finishBanner(hit: boolean, acc: number, leftHand = false, called: string | null = null): string {
    ...
    const hands = launchFoot === 'one' ? 'ONE-HAND' : 'TWO-HAND';
    if (acc >= 0.85) return `${hands} HAMMER!`;
    if (acc >= 0.55) return `${hands} FLUSH!`;
    return '';
  }
  ...
  function apexFor(): number { if (isDubble(prop)) return DUBBLE_APEX_M + doubleLaunchLift; return ((1.05 + charge * 0.55) * (0.85 + launchSpeed01 * 0.3) + apexLift()) * launchMult() + doubleLaunchLift; }   // DUNK PARKOUR: the launch's multiplier and the double-launch's lift
  ...
  /** DUNK MOTION phase 10: the Dubble Up's flight height (0..1 of DUBBLE_APEX_M) at clip `t` — explosive through the helper, the top at the rim. */
  function dubbleHeight(t: number): number {
    const endZ = rim.z + FLUSH_Z_AHEAD, helperZ = obstacle?.dubble?.holderZ ?? endZ;
    const uH = (launchZ - helperZ) / Math.max(0.1, launchZ - endZ);
    const u = (t - PLANT_SEC) / Math.max(0.1, EASTBAY_TIMING.extend - PLANT_SEC);
    if (u <= 0) return 0;
    return u >= 1 ? arcHeight(t, EASTBAY_TIMING.extend) : dubbleArc(u, dubbleArcPower(uH));
  }
  function apexPredicted(): number { if (isDubble(prop)) return DUBBLE_APEX_M; return ((1.05 + Math.max(charge, chargeAtLaunch) * 0.55) * (0.85 + Math.min(1, Math.max(runUpPeak, holdRunSpeed) / 7) * 0.3) + apexLift()) * launchMult() + doubleLaunchLift; }
  ...
  function curveOffset(dToLine: number): number {
    ...
    const d = Math.max(0, dToLine);
    const k = d >= CURVE_FROM_M ? 1 : 1 - (1 - d / CURVE_FROM_M) ** 2;
    return curveSide * (CURVE_LINE_M + (CURVE_WIDE_M - CURVE_LINE_M) * k);
  }
  ...
  /** DUNK MOTION phase 10 — the Dubble Up in the air: the ball taken, the hand's reach for it, the straddle, the helper passed, a trick
   *  pressed before the grab (it fires on the grab). */
  let dubbleGrabbed = false, dubbleReachK = 0, straddleK = 0, dubblePassed = false; let dubbleAir: DunkTrick | null = null;
  ...
  function slamNow(ctx: ModeContext, at: number): void {
    ...
    slamSeen = true;   // the latch has the press (SlamLatch.press / .open) — HOTFIX (2026-09-24)
  ...
  function bufferSlam(takeoffEcho = false): void {
    if (phase !== 'cinematic' || lob.live) return;
    // HOTFIX (2026-09-24): the take-off's own A (core/slamPress: the jump's, or a late "tap JUMP" after the line) is not the attempt's press
    if (takeoffEcho) { console.info(`[DUNK-SLAM] the take-off's own A @${clipTime.toFixed(2)} — not the slam`); return; }
    if (slamLatch.press(clipTime, false) === 'spent') { console.info(`[DUNK-SLAM] a second press @${clipTime.toFixed(2)} ignored — the first press decides`); return; }
    slamSeen = true;
   ...
   *  `takeoffEcho`: this A is the take-off's own (core/slamPress) — it may still arm a trick, it is never the slam. */
  function airButton(ctx: ModeContext, e: FelInput, takeoffEcho = false): void {
    const isA = e.t === 'button' && e.btn === 'A';
    // DUNK MOTION phase 10: the Dubble Up's trick is thrown WITH the ball — pressed before the grab, it fires on the grab
    // (read NOW, while the direction is held — replayed at the grab the d-pad was long released and nothing matched: the trick never fired)
    if (!isA && obstacle?.dubble && !dubbleGrabbed && phase === 'cinematic') { const t = flight.peek(e); if (t) { dubbleAir = t; console.info(`[DUBBLE] ${t.id} waits for the grab`); } return; }
    ...
    if (!trick || (isA && flight.recognizer.dirSpent)) {
      ...
      if (isA) { if (trick) console.info(`[DUNK-CUE] ${trick.id}'s direction is spent — the press is the SLAM`); bufferSlam(takeoffEcho); }
      return;
    }
    ...
    if (trick.id === 'spin360' && !isA) {
      ...
      if (airTrick?.trick.id === 'spin720') return;   // two turns is the whole flight's worth
    }
    // a second whole-body turn cannot start on top of one still turning (it restarted the yaw layer: a visible snap)
    if (!isA && cueOf(trick).facing === 'spinThrough' && spin.active && (spin.record.turns !== 0)) { refuse(ctx, 'ONE TURN AT A TIME — LET IT LAND'); return; }
    ...
    if (v === 'late') {
      ...
      if (isA) { bufferSlam(takeoffEcho); return; }   // the beat has gone; the button in his hand is still the slam
  ...
  function upgradeTo720(ctx: ModeContext): void {
    ...
    liveTricks.push({ clip: SPIN_720.clip, t0: clipTime, speed: rate });
    console.info(`[DUNK-TRICK] air spin720 @${clipTime.toFixed(2)} (the 360 thrown again: 2 turns, rim-facing by ${SPIN_RESOLVE_T.toFixed(2)})`);
    playAir(SPIN_720.clip, rate, 0.12);
    hype = Math.min(100, hype + 8);
    SoundKit.play('whoosh', { pitch: 1.1 + SPIN_720.difficulty * 0.08, volume: 0.5 });
    SoundKit.play('crowdCheer', { volume: 0.55 });
    flash(ctx, trickLabels.length > 1 ? `COMBO: ${trickLabels.join(' → ')}!` : '720!', 700);
    ctx.camDirector.pulse(0.6, 0.5);
  }
  ...
  function launchDunk(ctx: ModeContext, cause: LaunchCause = 'auto'): void {
    ...
    slamLatch.clear(); slamSeen = false; slamCueOn = false; beatCalled = false;   // DUNK-BODY-MID: the slam buffer is per attempt
    ...
    console.info(`[DUNK-LAUNCH] charge ${charge.toFixed(2)} run ${runUpPeak.toFixed(1)} apex ${apexFor().toFixed(2)} from z ${launchZ.toFixed(2)} to line ${gatherLine().toFixed(2)}${lob.live ? ' lob live' : ''}`);
    ...
    else if (isDubble(prop)) { /* the ball waits on the helper's head: the grab is the hand's job (dubbleTick) */ }
    ...
    playClip(launchClip(), { speedRatio: 1, onEnd: () => { if (phase === 'cinematic') { const c = carryClip(); console.info(`[HANDS] launch → ${c}`); playAir(c, carryRateToResolve(c), CARRY_FLOW_FADE_SEC); } } });
    // HOTFIX (2026-09-24): stamped LAST, once the launch's own work is done — a first launch that hitches (clips, the lob re-aim)
    // must not age the echo of an A pressed in the same frame as the take-off
    slamEcho.launched(performance.now(), cause);
  }
  ...
  // ── DUNK MOTION phase 10: THE DUBBLE UP in the air ──────────────────────────────────────────────────────────────────
  /** Over the helper: the legs open around his head, the ball hand comes down onto the ball and takes it, and the hips must pass over
   *  his head (a straddle — the feet go either side of him, so the clear is the HIPS against his head, not the feet). */
  function dubbleTick(ctx: ModeContext, dt: number): void {
    const d = obstacle!.dubble!; const dz = player.root.position.z - d.holderZ;   // + = still coming (the runway is +z of the rim)
    const want = Math.max(0, 1 - Math.max(0, Math.abs(dz) - 0.35) / 0.45);
    straddleK += (want - straddleK) * lowPassK(dt, 0.06);
    if (!dubbleGrabbed) {
      // the hand starts down for the ball by TIME, not distance: a long line crosses the ground at 5 m/s, and 1.1 m before him was 0.2 s —
      // the grab came after the body had passed him, 0.3–0.8 m from the ball (measured over 5 and 10)
      const carryMps = Math.abs(launchZ - (rim.z + FLUSH_Z_AHEAD)) / Math.max(0.2, EASTBAY_TIMING.extend - PLANT_SEC);
      if (dz < Math.max(DUBBLE_REACH_FROM_M, carryMps * DUBBLE_REACH_LEAD_SEC)) dubbleReachK = Math.min(1, dubbleReachK + dt / 0.14);
      const arm = arms[hs('Right')];
      if (arm) {
        arm.hand.computeWorldMatrix(true); ball.computeWorldMatrix(true);
        const gap = Vector3.Distance(arm.hand.getAbsolutePosition(), ball.getAbsolutePosition());
        if (gap < DUBBLE_GRAB_M || dz < -0.1) grabDubble(ctx, gap);
      }
    }
    if (!dubblePassed && dz <= 0) {
      dubblePassed = true; hipsNode?.computeWorldMatrix(true);
      const hy = hipsNode ? hipsNode.getAbsolutePosition().y : player.root.position.y + 0.96;
      if (hy < DUBBLE_HEAD_TOP + 0.04) {
        obstacleClipped = true; setTrail('off'); clipFloorY = 0; clipBackZ = Math.max(player.root.position.z, d.holderZ + 0.5);
        console.info(`[DUBBLE] CLIPPED the helper: hips ${hy.toFixed(2)} under his head ${DUBBLE_HEAD_TOP.toFixed(2)}`);
        clipBlown(ctx);
      } else {
        console.info(`[DUBBLE] over the helper: hips ${hy.toFixed(2)} vs his head ${DUBBLE_HEAD_TOP.toFixed(2)}`);
        // the single helper has no line behind him: he IS the clear; a line is cleared by its own hitbox once it is all behind the body
        if (!obstacleCleared && (obstacle!.spec.bodyCount ?? 1) <= 1) { obstacleCleared = true; onObstacleCleared(ctx); }
      }
    }
  }
  function grabDubble(ctx: ModeContext, gap: number): void {
    ball.computeWorldMatrix(true); const bw = ball.getAbsolutePosition().clone();
    attachBallToHand(ball, player.skeleton, hand('RightHand')); ebState.inLeftHand = hand('RightHand') === 'LeftHand';
    catchWorld.copyFrom(bw); catchPending = true; catchBlend = 0;   // eased from where the hand met it into the palm (the lob's own catch)
    dubbleGrabbed = true; dubbleReachK = 0;
    flash(ctx, 'DUBBLE UP!', 700); SoundKit.play('whoosh', { pitch: 1.2, volume: 0.6 }); hype = Math.min(100, hype + 8);
    console.info(`[DUBBLE] grabbed off his head (gap ${gap.toFixed(2)} m) @${clipTime.toFixed(2)}`);
    if ((obstacle?.spec.bodyCount ?? 1) <= 1) fireDubbleTrick(ctx);   // a single helper: the trick goes with the ball (a line waits for its clear)
  }
  /** THE DUBBLE UP'S TRICK goes with the ball, and over a line only once the line is behind the body: fired at the grab (and over the
   *  helper) its shape — one leg up, one hanging — put the low foot into the standing heads (clipped over 5 and 10, measured). Its timing
   *  is the Dubble Up's, not the cue table's: it only has to start before the slam window opens. */
  function fireDubbleTrick(ctx: ModeContext): void {
    if (!dubbleAir) return; const t = dubbleAir; dubbleAir = null;
    if (phase !== 'cinematic' || qteWindowOpen) { refuse(ctx, `TOO LATE FOR THE ${t.label}`); return; }
    console.info(`[DUBBLE] ${t.id} goes with the ball @${clipTime.toFixed(2)}`);
    fireTrick(ctx, t, 'window');
  }
  /** The ball hand down onto the TOP of the ball on the helper's head (it takes it from above), eased in over the approach to him. */
  function applyDubbleReach(): void {
    if (dubbleReachK <= 0.001 || dubbleGrabbed || !obstacle?.dubble || REACH_OFF) return;
    const side = hs('Right'), arm = arms[side]; if (!arm) return;
    ball.computeWorldMatrix(true); player.root.computeWorldMatrix(true);
    ...
    const k = dubbleReachK * dubbleReachK * (3 - 2 * dubbleReachK);
    _reachT.copyFrom(ball.getAbsolutePosition()); _reachT.y += 0.1;
    const want = hd.add(_reachT.subtract(hd).scale(k));
    ...
    const pole = anatomicalElbowPole(handIkPole, sh, want, Vector3.Up(), _reachFwd);
    const shaped = shapeReach(sh, el, hd, want, pole, undefined, REACH_POLE_CAP * k);
    reachArm(arm, shaped.target, shaped.pole, 1);
    limitElbowSwing(arm, motionDt(), ikFrame, REACH_SWING_RATE_DEG);
  }
  /** The straddle: each thigh swung out about the body's forward line, the way the foot goes AWAY from the other (checked, not assumed). */
  function applyStraddle(): void {
    if (straddleK <= 0.01 || MOTION_OFF) return;
    const ang = (STRADDLE_DEG * Math.PI / 180) * straddleK * straddleK * (3 - 2 * straddleK);
    player.root.computeWorldMatrix(true);
    _reachFwd.set(0, 0, 1).applyRotationQuaternionInPlace(player.root.absoluteRotationQuaternion); _reachFwd.y = 0; _reachFwd.normalize();
    const lf = feet.L, rf = feet.R; if (!lf || !rf) return;
    for (const [side, ft, other] of [['Left', lf, rf], ['Right', rf, lf]] as const) {
      const ul = boneNode(player.skeleton, `${side}UpLeg`); if (!ul) continue;
      ft.computeWorldMatrix(true); other.computeWorldMatrix(true);
      const apart0 = Vector3.Distance(ft.getAbsolutePosition(), other.getAbsolutePosition());
      ul.rotate(_reachFwd, ang, Space.WORLD);
      const walk = (n: TransformNode) => { n.computeWorldMatrix(true); for (const c of n.getChildTransformNodes(true)) walk(c); }; walk(ul);
      if (Vector3.Distance(ft.getAbsolutePosition(), other.getAbsolutePosition()) < apart0) { ul.rotate(_reachFwd, -2 * ang, Space.WORLD); walk(ul); }
    }
  }
  ...
  function handIkApply(): void {
    ...
    applyDubbleReach();    // DUNK MOTION phase 10: the hand down onto the ball on the helper's head
    applyStraddle();       // …and the legs open around him
    ...
    // (the Dubble Up: no reach for the iron until the ball is off the helper's head — over a long line he is passed at the carry-up, and the
    // rim reach pulled the ball hand up away from the ball it was going down for: the grab came after him, 0.8 m from the ball)
    if (w > 0.001 && !REACH_OFF && !(obstacle?.dubble && !dubbleGrabbed)) {
      player.root.computeWorldMatrix(true);
  ...
  function footNow(): 'one' | 'two' { return isDubble(prop) ? 'two' : takeoffFor(Math.max(runUpPeak, holdRunSpeed), gatherHeld); }   // the Dubble Up goes off two (Chen)
  /** The take-off clip: the POWER flight goes up off the foot the run-up chose — the one-foot take-off (the knee drive, the arm
   *  strike, the lead leg's drop) or the owner's own two-foot capture. Flashy / signature keep their own. */
  function launchClip(): string {
    return style === 'power' && MOCAP_DUNK && launchFoot === 'one' ? 'dunk_take_off_one' : STYLE_CLIP[style];
  }
  ...
  function resetRunway(): void { teachHint = ''; pendingBeat = null; runwayBeat = null; runwayT = 0; runwayReleased = false; runwayLabels = []; runwayDifficulty = 0; launchQueued = false; airTrick = null; runwayToken++; }
  ...
  function startRunwayBeat(ctx: ModeContext, rt: RunwayTrick): void {
    ...
    flash(ctx, rt.label, 650);
    runwayLabels.push(rt.label); runwayIds.push(rt.id); runwayDifficulty += rt.difficulty;
    SoundKit.play('whoosh', { pitch: rt.id === 'handspring' || rt.id === 'cartwheel' ? 0.8 : 1.2, volume: 0.4 });
    ctx.camDirector.pulse(0.3, 0.35);
    // the kick-up drops the ball to the foot: it leaves the hand at once and falls ONTO the kicking foot for the contact (DUNK MOTION
    // phase 10b — it used to free-fall to the floor a stride away while the foot kicked the air, and the lob left from the floor)
    if (rt.id === 'kickup') {
      releasePos.copyFrom(ball.getAbsolutePosition()); releaseBall(ball); ballSim.stop();
      kickDropFrom.copyFrom(releasePos).subtractInPlace(player.root.position);
      lob.live = true; lob.thrown = true; lob.label = rt.label;
    }
  }
  const kickDropFrom = new Vector3(), _kickFoot = new Vector3();
  /** Where the kicking foot holds the ball (world): over the instep, a ball's radius up — the right foot, mirrored onto the rig. */
  function kickFootPoint(out: Vector3): Vector3 | null {
    const foot = boneNode(player.skeleton, `${hs('Right')}Foot`), toe = boneNode(player.skeleton, `${hs('Right')}ToeBase`);
    if (!foot) return null;
    foot.computeWorldMatrix(true); out.copyFrom(foot.getAbsolutePosition());
    if (toe) { toe.computeWorldMatrix(true); Vector3.LerpToRef(out, toe.getAbsolutePosition(), 0.6, out); }
    out.y += BALL_R_KICK;
    return out;
  }
  /** Phase 10b: the kick-up's ball, from the hand onto the foot — falling (eased in, like gravity) with the runner, landing on the
   *  instep at the contact. Runs every frame of the beat until the kick. */
  function kickDropTick(rt: RunwayTrick): void {
    if (rt.id !== 'kickup' || runwayReleased || rt.releaseAt == null) return;
    const to = kickFootPoint(_kickFoot); if (!to) return;
    const u = clamp(runwayT / rt.releaseAt, 0, 1), k = u * u;
    const x = player.root.position.x + kickDropFrom.x, y = player.root.position.y + kickDropFrom.y, z = player.root.position.z + kickDropFrom.z;
    ball.position.set(x + (to.x - x) * k, y + (to.y - y) * k, z + (to.z - z) * k);
  }
  function runwayTick(ctx: ModeContext, dt: number): void {
    ...
    if (rt.releaseAt != null && !runwayReleased && runwayT >= rt.releaseAt) {
      runwayReleased = true;
      if (rt.id === 'kickup') { ballSim.stop(); if (kickFootPoint(_kickFoot)) ball.position.copyFrom(_kickFoot); SoundKit.play('uiTick', { pitch: 0.6, volume: 0.5 }); }   // the flick: it leaves from the foot
      const from = ball.getAbsolutePosition().clone();
      ...
      else if (rt.id === 'cartwheel') throwBounceLob(ctx, from, tLaunch + LOB_CATCH_CLIP_T, phase !== 'charge', dist);   // phase 10b: Jus Fly throws it into the floor and takes it off the bounce
      else throwLob(ctx, from, rt.id === 'handspring' ? 'SELF-LOB' : rt.label, tLaunch + LOB_CATCH_CLIP_T);
    }
  ...
  function onObstacleCleared(ctx: ModeContext): void {
    ...
    if (obstacle.dubble && dubbleGrabbed) fireDubbleTrick(ctx);   // DUNK MOTION phase 10: the line is behind him — the trick goes now
  }
  ...
  function contactPunch(ctx: ModeContext): void {
    ...
    contactLatch = true;
    ctx.juice.hitStop(70, { gameplay: true });   // HOTFIX (2026-09-24): paired with the gameplay freeze below — reduced motion keeps both 70 ms, so the limbs never move while the root and ball hold
