...
import { collectBalloon, balloonsHit, fireItem, stepMissiles, stepMines, bananasAfterHit, type Balloon, type Missile, type Mine } from './AeroItems';
...
describe('items', () => {
  ...
  it('level 1 missile flies straight, level 2 homes, level 3 fires three', () => {
    expect(fireItem({ kind: 'missile', level: 1 }, 0, from, aim, back, 2).missiles[0].homing).toBe(false);
    expect(fireItem({ kind: 'missile', level: 2 }, 0, from, aim, back, 2).missiles[0].target).toBe(2);
    expect(fireItem({ kind: 'missile', level: 3 }, 0, from, aim, back, 2).missiles.length).toBe(3);
  });

  it('boost and shield scale with level; mines drop behind', () => {
    expect(fireItem({ kind: 'boost', level: 3 }, 0, from, aim, back, null).boostSec).toBeGreaterThan(fireItem({ kind: 'boost', level: 1 }, 0, from, aim, back, null).boostSec);
    expect(fireItem({ kind: 'shield', level: 2 }, 0, from, aim, back, null).shieldSec).toBe(6);
    const mines = fireItem({ kind: 'mine', level: 3 }, 0, from, aim, back, null).mines;
  ...
  it('a homing missile catches a racer that moved off its first line; a shield absorbs it', () => {
    ...
    const ms: Missile[] = fireItem({ kind: 'missile', level: 2 }, 0, from, aim, back, 1).missiles;
    let hit: number[] = [];
    for (let i = 0; i < 120 && !hit.length; i++) hit = stepMissiles(ms, [target], 1 / 60).hit;
    expect(hit).toEqual([1]);
    const ms2: Missile[] = fireItem({ kind: 'missile', level: 2 }, 0, from, aim, back, 1).missiles;
    let absorbed: number[] = [];
  ...
  it('you cannot hit your own missile, and a mine arms before it can hit', () => {
    const ms: Missile[] = fireItem({ kind: 'missile', level: 1 }, 0, from, aim, back, null).missiles;
