...
interface Row { key: string; enabled: boolean; file: string | null; route: string; menu: boolean; venue: string | null; map: string | null; kit: string | null; baked: boolean; cores: string[]; camera: boolean; clamps: number; bridge: boolean; gauntlet: boolean; play: boolean; lock: boolean; tests: number; lines: number }
const rows: Row[] = modeEntries.map(({ key, cls }) => {
  const file = modeFile(cls); const src = file ? read(file) : '';
...
for (const r of rows) lines.push(`| ${r.key} | ${mark(r.enabled)} | ${r.route} | ${mark(r.menu)} | ${r.venue ?? '—'} → ${r.map ?? '—'} | ${r.kit ?? '—'} | ${mark(r.baked)} | ${r.cores.join(', ') || '—'} | ${mark(r.camera)} | ${r.clamps} | ${mark(r.bridge)} | ${mark(r.gauntlet)} | ${mark(r.play)} | ${mark(r.lock)} | ${r.tests} | ${r.lines} |`);
const gaps: string[] = [];
