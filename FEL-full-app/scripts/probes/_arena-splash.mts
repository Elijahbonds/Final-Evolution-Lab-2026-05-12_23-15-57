...
const BASE = process.env.BASE ?? 'http://127.0.0.1:3098', MODE = process.env.MODE ?? 'duel', ARENA = process.env.ARENA ?? '', QS = process.env.QS ?? '';
...
await p.goto(`${BASE}/dev/mode/${MODE}?agent=1${ARENA ? `&arena=${ARENA}` : ''}${QS}`, { waitUntil: 'domcontentloaded', timeout: 240000 });
await p.waitForSelector('text=ARENA', { timeout: 240000 }).catch(() => console.log('no ARENA chips'));
await p.waitForTimeout(1500);
await p.screenshot({ path: `${OUT}/${MODE}-${ARENA || QS.replace(/[^a-z-]/g, '') || 'default'}-splash.png` });
...
await p.screenshot({ path: `${OUT}/${MODE}-${ARENA || QS.replace(/[^a-z-]/g, '') || 'default'}-play.png` });
console.log('logs:\n  ' + logs.join('\n  '));
await b.close();
