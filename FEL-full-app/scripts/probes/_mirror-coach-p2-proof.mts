// MIRROR-COACH P2 (2026-09-26) — THE PHASE'S LIVE PROOF, Mirror side: a copy of _mirror-p2-closeout-live.mts run on the
// final P2 tree, with two additions: a frame the moment the FIRST rep is counted (reps 0 → 1, with the fixture loop it
// fell in), and a third square case — knees caving 6 cm but 8° off square — which must NOT be cued (the cue speaks on a
// square knee-in only). The screen run is left out (P3 owns the screen).
//
...
const OUT = process.argv[2] ?? '/tmp/mirror-p2-proof';
...
async function guidedSquat(browser: Browser, f: Json, shotName: string | null): Promise<Json> {
  ...
  try {
    ...
    while (Date.now() - t0 < 180_000) {
      ...
      // the first rep the page counts: the frame, the stage, and which fixture loop (= which squat) it fell in
      if (!rec.firstRep && typeof r.reps === 'number' && r.reps >= 1) {
        rec.firstRep = { ms: Date.now() - t0, stage: r.stage, reps: r.reps, loop: r.loop };
        if (shotName) {
          await page.screenshot({ path: join(OUT, `${shotName}-rep1.png`) });
          out.frames[`${shotName}-rep1.png`] = { url: '/dev/mirror-truth', what: `${f.name}: the moment the first rep is counted (${r.stage}, reps ${r.reps}, fixture loop ${r.loop})` };
        }
      }
...
try {
  if (want('squat')) {
    for (const name of (process.env.SQUAT_ONLY?.split(',') ?? ['squat_knee_in_left', 'squat_knees_out_both', 'squat_clean'])) {
      const r = await guidedSquat(browser, forPage(name), name.replace(/_/g, '-'));
  ...
  if (want('square')) {
    for (const [name, shape, shot] of [
      ...
      ['synth_knees_in_6cm_8deg_off_square_seed3', { shiftL: -0.06, shiftR: -0.06, turnDeg: 8 }, 'square-knees-in-8deg-off'],
