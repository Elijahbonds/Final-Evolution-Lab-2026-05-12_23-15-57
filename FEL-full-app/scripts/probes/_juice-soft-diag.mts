...
p.on('console', (m) => { const x = m.text(); if (x.includes('[JUICE-SOFT]')) events.push(`${((Date.now() - t0) / 1000).toFixed(1)}s ${x.replace('[JUICE-SOFT] ', '')}`); });
...
events.push('--- attempt 1 (slam)'); await attempt(true);
events.push('--- attempt 2 (no slam → miss)'); await attempt(false);
