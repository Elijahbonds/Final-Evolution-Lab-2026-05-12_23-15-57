...
const kFrom = raw.find((k) => k.t >= CARRY.from - 1e-6), kTo = raw.find((k) => k.t >= CARRY.to - 1e-6);
if (kFrom && kTo && kTo.t > kFrom.t) {
  const a = kFrom.rh, b = kTo.rh;
  for (const k of raw) if (k.t > kFrom.t && k.t < kTo.t) {
    const u = (k.t - kFrom.t) / (kTo.t - kFrom.t);
    k.rh = [a[0] + (b[0] - a[0]) * u, a[1] + (b[1] - a[1]) * u, a[2] + (b[2] - a[2]) * u];
  }
}
