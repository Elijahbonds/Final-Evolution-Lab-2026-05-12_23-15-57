...
/**
 * Clips measured below tolerance in the authored pose but intentionally not foot-planted,
 * with the thing that owns floor contact named. Anything not on this list has to stand on its own.
 *
 * This list should stay tiny. It briefly held dunk_mocap on the claim that basketballTree "plants
 * the dunk legs" — which was read off an import rather than measured. That plant pins the ankle's X
 * and Z and explicitly KEEPS THE CLIP'S HEIGHT (`target.set(pin.x, ankle.getAbsolutePosition().y,
 * pin.z)`), so it corrects nothing vertically and the dunk's opening frame really did put a boot
 * through the court. An exemption here needs a measured floor-contact test, not an import.
 */
const RUNTIME_PLANTED: Record<string, string> = {
  bball_ankle_slip: 'authored/basketball.test.ts verifies the hip stays down and the hand reaches the floor',
};
...
console.log(`  ✓ ${Object.keys(RUNTIME_PLANTED).length} measured floor-contact clip(s) exempt`);
