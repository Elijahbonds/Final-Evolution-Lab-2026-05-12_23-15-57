...
for (const mesh of root.listMeshes()) for (const prim of mesh.listPrimitives()) {
  ...
  if (/^(jersey|shorts|shoes|hair)\.[A-Za-z0-9_-]+/.test(mat.getName())) {   // kit garment / hair: <slot>.<key> stays (Blender's .001 suffix dropped)
...
for (const n of root.listNodes()) { if (/^(Kit|Hair)_/.test(n.getName())) continue; const m = n.getMesh(); if (m) { const mats = m.listPrimitives().map((p) => p.getMaterial()?.getName()).filter(Boolean); if (mats.length === 1 && mats[0] !== 'skin') n.setName(mats[0]!); else if (mats[0] === 'skin') n.setName('Body'); } }
